...
本模型先由邏輯式回歸得到**預測機率 \(P\)**（病人發生 IHCA 之估計機率），再依 \(P\) 所落區間歸為低、中、高風險，以利臨床分層與溝通。分級**僅依 \(P\) 的數值**，使用兩個切點：

- **切點 0.10（10%）**：\(P < 0.10\) 視為**低風險**。預測機率低於 10% 時，在風險分層上常對應常規監測與照護即可，故訂為低風險之上限。
- **切點 0.30（30%）**：\(P \ge 0.30\) 視為**高風險**。預測機率達 30% 以上時，臨床上多列為高風險族群，建議加強監測或積極處置；介於 10% 與 30% 之間則歸為**中度風險**。

簡言之：**邏輯回歸只產出機率 \(P\)；三種風險是依 \(P\) 的區間（< 0.10、0.10–0.30、≥ 0.30）對應低、中、高風險，供論文引用或向臨床人員說明分級依據。**

| 風險等級 | 預測機率 \(P\) | 臨床解讀（供參考） |
|----------|----------------|---------------------|
| 低風險（Low） | \(P < 0.10\) | 預測發生 IHCA 機率較低，可依常規監測與照護。 |
| 中度風險（Moderate） | \(0.10 \le P < 0.30\) | 預測機率中等，建議提高警覺、加強監測與評估。 |
| 高風險（High） | \(P \ge 0.30\) | 預測機率較高，建議積極監測、及早討論照護目標與處置。 |

*臨床決策仍應以主治醫師判斷為準，本表僅供說明分級邏輯。*
